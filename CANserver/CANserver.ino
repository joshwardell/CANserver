...
static int UIChargeStatus = 0; //3 if charging, used to start the charge log
static int DI_gear = 0; //2 if reverse, 4 if drive, used to start drive log
static int UTCyear = 0; 
static int UTCmonth = 0;
static int UTCday = 0;
static int UTChour = 0;
static int UTCminute = 0;
static int UTCsecond = 0;
static int UTCTimeZoneOffset = 4; //4 for Eastern USA
...
unsigned long previousLogcycle = 0;
static int driveLogInterval = 5000; //how often the drive log is written to in ms
static int chargeLogInterval = 5000; //how often the charfge log is written to in ms
...
{
    ArduinoOTA.handle();
    long currentLogMillis = millis();
    if (currentLogMillis - previousLogcycle >= driveLogInterval && (DI_gear == 2 || DI_gear == 4) && SD.cardType() != CARD_NONE) //if you have an SD card and are in drive or reverse, write to the drive log every driveLogInterval ms
    {
        previousLogcycle = currentLogMillis;
        appendFile(SD, "/driveLog.txt", String(UTCmonth) + "/" + String(UTCday) + "/" + String(UTCyear) + "," + String(UTChour) + ":" + String(UTCminute) + ":" + String(UTCsecond) + "," + String(BattVolts) + "," + String(BattAmps) + "," + String(BattPower) + "," + String(RearTorque) + "," + String(MinBattTemp) + "," + String(MaxRegen) + "," + String(MaxDisChg) + "," + String(VehSpeed) + "," + String(BattRemainKWh) + "," + String(BattFullKWh) + "\n"); //write comma seperate data and end the line to move to new row
    }
    if (currentLogMillis - previousLogcycle >= chargeLogInterval && UIChargeStatus == 3 && SD.cardType() != CARD_NONE) //if you have an SD card and are charing, write to the charge log every chargeLogInterval ms
    {
        previousLogcycle = currentLogMillis;
        appendFile(SD, "/chargeLog.txt", String(UTCmonth) + "/" + String(UTCday) + "/" + String(UTCyear) + "," + String(UTChour) + ":" + String(UTCminute) + ":" + String(UTCsecond) + "," + String(BattVolts) + "," + String(BattAmps) + "," + String(BattPower) + "," + String(RearTorque) + "," + String(MinBattTemp) + "," + String(MaxRegen) + "," + String(MaxDisChg) + "," + String(BattRemainKWh) + "," + String(BattFullKWh) + "\n"); //write comma seperate data and end the line to move to new row
    }
    if (Serial)
    {
...
*/
        ...
        case 0x118:
            if (message.length == 8)
            {
                DI_gear = ((1 << 3) - 1) & (message.data.uint64 >> 21); //mask unwanted bits and shift
            }
            break;
        ...
        case 0x212:
            if (message.length == 8)
            {
                UIChargeStatus = ((1 << 3) - 1) & (message.data.uint64 >> 11);
            }
            break;
...
*/
        ...
        case 0x318:
            if (message.length == 8)
            {
                UTCyear = ((1 << 8) - 1) & (message.data.uint64 >> 0);
                UTCmonth = ((1 << 8) - 1) & (message.data.uint64 >> 8);
                UTCday = ((1 << 8) - 1) & (message.data.uint64 >> 32);
                UTChour = ((1 << 8) - 1) & (message.data.uint64 >> 24);
                UTCminute = ((1 << 8) - 1) & (message.data.uint64 >> 40);
                UTCsecond = ((1 << 8) - 1) & (message.data.uint64 >> 16);
                if (UTChour - UTCTimeZoneOffset < 0)
                {
                    UTChour = (24 - abs(UTChour - UTCTimeZoneOffset));
                    UTCday = UTCday - 1;
                    if (UTCday <= 0){
                        UTCmonth = UTCmonth -1;
                        switch(UTCmonth){
                ...
                {
                    UTChour = (UTChour - UTCTimeZoneOffset);
                }
            }
